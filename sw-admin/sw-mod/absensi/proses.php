...
<table class="table table-bordered table-hover" id="swdatatable">
      ...
      $query_absen ="SELECT presence_id,presence_date,building_id,time_in,time_out,picture_in,picture_out,present_id, latitude_longtitude_in,latitude_longtitude_out,information,TIMEDIFF(TIME(time_in),'$shift_time_in') AS selisih,if (time_in>'$shift_time_in','Telat',if(time_in='00:00:00','Tidak Masuk','Tepat Waktu')) AS status FROM presence WHERE $filter ORDER BY presence_id DESC";
      ...
      $query_building  ="SELECT name,latitude_longtitude,radius FROM building WHERE building_id='$row_absen[building_id]'";
      ...
      $row_building =  $result_building->fetch_assoc();
        ...
        echo'
         <tr style="background:'.$background.';color:'.$warna.'">
            ...
            <td>'.strip_tags($row_building['name']).'</td>';
            if (date("l",mktime (0,0,0,$bulan,$d,$tahun)) == "Sunday"){
              if($row_absen['time_in'] ==''){
                echo'
                <td class="text-center picture">';
                  if($row_absen['picture_in'] ==NULL){
                    echo '<img src="../sw-content/nophoto.jpg" height="40" width="40">';}
                   // echo'<img src="../timthumb?src='.$site_url.'/sw-content/nophoto.jpg&h=40&w=40">';}
                  else{
                    echo'
                      <a class="image-link" href="../sw-content/present/'.$row_absen['picture_in'].'" target="_blank">
                      <img src="../sw-content/present/'.$row_absen['picture_in'].'" height="40" width="40"></a>';}
                    echo'</td>
            ...
            <td class="text-center picture">';
              if($row_absen['picture_in'] ==NULL){
                ...
                echo'
                <a class="image-link" href="../sw-content/present/'.$row_absen['picture_in'].'" target="_blank">
                <img src="../sw-content/present/'.$row_absen['picture_out'].'" height="40" width="40"></a>';}
            echo'</td>
            <td>'.$row_absen['time_in'].'</td>';
            }
            echo'
               ...
               echo'
                <td class="text-center picture">';
                  ...
                  //  echo'<img src="../timthumb?src='.$site_url.'/sw-content/nophoto.jpg&h=40&w=40">';}
                    ...
                    echo'
                    <a class="image-link" href="../sw-content/present/'.$row_absen['picture_out'].'" target="_blank">
                    <img src="../sw-content/present/'.$row_absen['picture_out'].'" height="40" width="40"></a>';}
                    echo'</td>
            ...
            echo'
            <td><p class="btn-modal" data-latitude="'.$latitude_out.'" data-longitude="'.$longitude_out.'">Lat: '.$latitude_out.'<br>Long: '.$longitude_out.'</p></td>
